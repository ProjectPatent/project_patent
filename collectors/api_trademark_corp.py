...
class AsyncTrademarkDownloaderCorp:
    def __init__(self):
                ...
                async with conn.cursor() as cursor:
                    ...
                    await cursor.execute("""
                        SELECT COUNT(DISTINCT applicant_no) FROM (
                            SELECT applicant_no FROM tb24_200_corp_applicant
    ...
    async def process_all(self):
        ...
        try:
            ...
            output_path = f'data/{date_str}_trademark_corp.xml'
            ...
            date_str = datetime.now().strftime('%Y%m%d')
            output_path = f'data/{date_str}_trademark_corp_interrupted.xml'
            self.xml_builder.save(output_path)
