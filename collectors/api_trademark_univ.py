...
class AsyncTrademarkDownloaderUniv:
    def __init__(self):
        ...
        # XML 빌더
        self.xml_builder = TrademarkXMLBuilder()
        ...
        if self.session is None or self.session.closed:
            ...
            self.session = aiohttp.ClientSession(
                ...
                headers={
                    ...
                    'User-Agent': 'TrademarkDownloader/1.0'
                }
            )
    ...
    async def get_trademark_params(self, applicant_no: str) -> Dict[str, str]:
        """상표 검색 파라미터 생성"""
        params = {
            'applicantName': applicant_no,
            'ServiceKey': self.service_key
            }
        params.update(api_input_params['trademark'])
        return
    ...
    async def fetch_trademark_data(self, applicant_no: str) -> Optional[str]:
        """상표 데이터 조회"""
        with self.metrics.api_request_duration.time():
            ...
            try:
                params = await self.get_trademark_params(applicant_no)
    ...
    async def process_single_applicant(self, applicant_no: str):
        """단일 출원인 처리"""
        try:
            xml_content = await self.fetch_trademark_data(applicant_no)
            if not xml_content:
                return
            
            with self.metrics.xml_processing_duration.time():
                if await self.xml_builder.add_trademark_data(xml_content, applicant_no):
    ...
    async def process_all(self):
        ...
        try:
            ...
            output_path = f'data/{date_str}_trademark_univ.xml'
            ...
            date_str = datetime.now().strftime('%Y%m%d')
            output_path = f'data/{date_str}_trademark_univ_interrupted.xml'
            self.xml_builder.save(output_path)
