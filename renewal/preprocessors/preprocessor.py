...
from config.api_config import API_OUTPUT_PARAMS_TO_PARSE
...
class DataParser():
    ...
    def ipc_cpc_parser(self, item):
        ...
        temp = {}
        for key, value in API_OUTPUT_PARAMS_TO_PARSE['patent_utility'].items():
    ...
    def priority_parser(self, service_type, item):
        ...
        temp = {}
        for key, value in API_OUTPUT_PARAMS_TO_PARSE[service_type].items():
            if value == 'applicationNumber':
                appl_no = str(item.find(f'.//{value}').text)
                temp[f'{key}'] = appl_no
